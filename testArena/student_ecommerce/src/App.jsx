...
function App() {
 ...
 const handleAddToCart = (name, price, qty, waist, length) => {
  setCart((prev) => {
    let updated = addItem(prev, name, price, qty, waist, length)
    const lightWashItem = updated.find(item => item.name.toLowerCase().includes('light wash'))
    const lightWashQty = lightWashItem ? lightWashItem.quantity : 0
    if (lightWashQty >= 2) {
      updated = addItem(updated, 'Free Belt', 0, 1)
    } else {
      updated = updated.filter(item => item.name !== 'Free Belt')
    }
    return saveItems(updated)
  })
  setCartOpen(true)
}
